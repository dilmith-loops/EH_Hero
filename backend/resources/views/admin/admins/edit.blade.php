...
@section('subtitle', 'Update administrator account credentials and password')
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-xs">
        <div class="flex items-center gap-3 pb-5 border-b border-slate-100 mb-6">
            ...
            <div>
                <h3 class="text-sm font-extrabold text-slate-900 uppercase tracking-wider">Edit Administrator: {{ $admin->name }}</h3>
                <p class="text-xs text-slate-400">Update account info or enter a new password below.</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('admin.admins.update', $admin->id) }}" class="space-y-5">
            ...
            <!-- Password Update Notice Box -->
            <div class="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 text-slate-600 text-xs flex items-center gap-2">
                <span class="text-base">ℹ️</span>
                <span class="font-medium">Leave password fields blank if you do not wish to change the password.</span>
            </div>

            <!-- New Password with Eye Toggle -->
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">New Password (Optional)</label>
                <div class="relative">
